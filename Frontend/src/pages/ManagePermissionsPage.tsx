...
export const ManagePermissionsPage: React.FC = () => {
  ...
  // Active vs all roles toggle
  const [roleFilter, setRoleFilter] = useState<'all' | 'active'>('all');

  // Filter roles and modules based on search and active toggle
  const filteredRoles = useMemo(() => {
    if (!data?.catalog.roles) return [];
    let roles = data.catalog.roles;
    if (roleFilter === 'active' && data.activeRoles && data.activeRoles.length > 0) {
      roles = roles.filter((r) => data.activeRoles.includes(r));
    }
    if (!searchQuery.trim()) return roles;
    const q = searchQuery.toLowerCase();
    return roles.filter((r) => r.toLowerCase().includes(q));
  }, [data?.catalog.roles, data?.activeRoles, roleFilter, searchQuery]);
  ...
  return (
    <div className="permissions-page px-4 sm:px-6 lg:px-8 py-6 max-w-[1600px] mx-auto space-y-6 select-none">
      ...
      >
        <div>
          <div className="flex items-center gap-2 mb-2">
            <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-[#fff7ed] border border-[#fed7aa] text-[#ea580c] text-xs font-semibold">
      ...
      {/* ── Super Admin Info Callout & Role Filter ── */}
      <motion.div
      ...
      >
        <div className="flex items-center gap-3">
          <div className="w-8 h-8 rounded-xl bg-[#fff7ed] border border-[#fed7aa]/40 flex items-center justify-center text-[#f97316] flex-shrink-0 shadow-2xs">
          ...
          <div>
            ...
            <p className="text-[11px] text-[#6b6b6b]">
              The <span className="text-[#f97316] font-mono font-semibold">super_admin</span> role
        ...
        <div className="flex flex-wrap items-center gap-2 flex-shrink-0">
          <div className="flex items-center gap-1 p-0.5 bg-white rounded-lg border border-[#ece0d6] shadow-2xs">
            <button
              type="button"
            ...
            >
              All Roles ({data?.catalog.roles.length ?? 0})
            </button>
            <button
              type="button"
            ...
            >
              Active Accounts ({data?.activeRoles?.length ?? 0})
            </button>
          </div>

          <div className="text-[11px] text-[#6b6b6b] font-mono bg-white px-3 py-1.5 rounded-lg border border-[#ece0d6] shadow-2xs">
            Staged Diffs: <span className={dirtyCells.size > 0 ? 'text-[#f97316] font-bold' : 'text-emerald-700 font-semibold'}>{dirtyCells.size}</span>
          </div>
        </div>
      </motion.div>
      ...
      {isLoading ? (
        ...
        <div className="p-10 rounded-2xl bg-[#fff7ed] border border-[#fed7aa] text-center max-w-lg mx-auto my-8 shadow-xs">
          ...
          <button
            type="button"
            onClick={() => refetch()}
            className="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-semibold text-white bg-[#f0512f] hover:bg-[#ff7a59] transition-colors shadow-md shadow-[#f0512f]/30 cursor-pointer"
          >
