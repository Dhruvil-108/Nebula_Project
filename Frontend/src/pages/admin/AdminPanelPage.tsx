...
// Recharts palette — warm oranges, emeralds, ambers (theme-consistent)
const C = {
  orange: '#C2540C',
  orangeLight: '#DE7A3D',
  peach: '#F6C6A2',
...
const StatTile: React.FC<{
  ...
  <div className={`${cardCls} p-5 relative overflow-hidden`}>
    <div className="absolute -top-8 -right-8 w-24 h-24 rounded-full bg-[#FBEAE0] blur-2xl pointer-events-none" />
    <div className="flex items-center justify-between mb-3 relative">
      ...
      <div
        className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 ${
          accent ? 'bg-[#C2540C] text-white' : 'bg-[#FBEAE0] text-[#C2540C]'
        }`}
      >
        {icon}
      </div>
  ...
  return (
    <span
      className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold border capitalize ${
        map[status] || 'bg-[#FBEAE0] text-[#7A2F05] border-[#de7a3d]'
      }`}
    >
      {status}
    </span>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
        <div className={`${cardCls} p-5 lg:col-span-8`}>
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              <Activity className="w-4 h-4 text-[#C2540C]" />
              <h4 className="text-sm font-semibold text-[#1A1A1A]">
                Attendance Trend — Last 7 Days
              </h4>
            </div>
            <span className="text-xs font-mono px-2 py-0.5 rounded bg-[#FBEAE0] text-[#7A2F05] font-semibold">
        ...
        <div className={`${cardCls} p-5 lg:col-span-4`}>
          <div className="flex items-center gap-2 mb-2">
            <UserCheck className="w-4 h-4 text-[#C2540C]" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
        <div className={`${cardCls} p-5 lg:col-span-8`}>
          <div className="flex items-center gap-2 mb-4">
            <TrendingUp className="w-4 h-4 text-[#C2540C]" />
            <h4 className="text-sm font-semibold text-[#1A1A1A]">
          ...
          <div className="h-52">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={overview.accountGrowth} margin={{ top: 5, right: 8, left: -18, bottom: 0 }}>
                ...
                <XAxis dataKey="label" tick={{ fontSize: 11, fill: '#6B6B6B' }} axisLine={false} tickLine={false} />
                <YAxis tick={{ fontSize: 11, fill: '#9B9B9B' }} axisLine={false} tickLine={false} allowDecimals={false} />
                <Tooltip content={<ChartTip />} cursor={{ fill: '#FBEAE0', opacity: 0.5 }} />
        ...
        <div className={`${cardCls} p-5 lg:col-span-4`}>
          <div className="flex items-center gap-2 mb-2">
            <CalendarCheck className="w-4 h-4 text-[#C2540C]" />
      ...
      <div className={`${cardCls} p-5`}>
        <div className="flex items-center gap-2 mb-4">
          <Users className="w-4 h-4 text-[#C2540C]" />
            ...
            <span
              ...
              className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-semibold border ${
                role === 'super_admin'
                  ...
                  : 'bg-[#FBEAE0] text-[#7A2F05] border-[#de7a3d]/50'
              }`}
            >
      ...
      {isSuper && overview.superAdmin && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
          <div className={`${cardCls} p-5 lg:col-span-7`}>
            <div className="flex items-center gap-2 mb-4">
              <LogIn className="w-4 h-4 text-[#C2540C]" />
          ...
          <div className={`${cardCls} p-5 lg:col-span-5`}>
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                <ShieldAlert className="w-4 h-4 text-[#C2540C]" />
                <h4 className="text-sm font-semibold text-[#1A1A1A]">Top Audit Actions</h4>
              </div>
              <button
                type="button"
                onClick={onOpenAudit}
                className="text-xs font-semibold text-[#C2540C] hover:text-[#D06B28]"
              >
                Full trail →
              </button>
            </div>
                  ...
                  return (
                    <div key={a.action}>
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <span className="text-[11px] font-bold text-[#C2540C]">{a.count}</span>
                      </div>
                      <div className="h-1.5 rounded-full bg-[#FBEAE0] overflow-hidden">
                        <motion.div
                          initial={{ width: 0 }}
                          animate={{ width: `${(a.count / max) * 100}%` }}
                          transition={{ duration: 0.6, ease: 'easeOut' }}
                          className="h-full rounded-full bg-gradient-to-r from-[#DE7A3D] to-[#C2540C]"
                        />
                      </div>
                    </div>
                  );
      ...
      {isSuper && overview.superAdmin && overview.superAdmin.recentAudit.length > 0 && (
        <div className={`${cardCls} p-5`}>
          <div className="flex items-center gap-2 mb-4">
            <ScrollText className="w-4 h-4 text-[#C2540C]" />
            ...
            {overview.superAdmin.recentAudit.map((log) => (
              ...
              >
                <div className="flex items-center gap-2 min-w-0">
                  <span className="text-[10px] font-mono font-semibold px-1.5 py-0.5 rounded bg-white border border-[#de7a3d]/40 text-[#7A2F05] flex-shrink-0">
                    {log.action}
                  </span>
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row gap-2.5">
        ...
        <button
          type="button"
          onClick={() => refetch()}
          className="p-2 rounded-xl text-[#6B6B6B] hover:text-[#1A1A1A] hover:bg-[#FBEAE0] border border-[#ECE0D6] transition-colors self-start"
          title="Refresh"
        >
                    ...
                    >
                      <td className="px-5 py-3.5">
                        <div className="flex items-center gap-2.5">
                          <div className="w-8 h-8 rounded-full bg-[#FBEAE0] border border-[#de7a3d]/40 flex items-center justify-center text-[11px] font-bold text-[#7A2F05] flex-shrink-0">
                          ...
                          <div className="min-w-0">
                            <p className="text-sm font-medium text-[#1A1A1A] flex items-center gap-1.5">
                              {u.fullName}
                              {isSelf && (
                                <span className="text-[9px] px-1.5 py-0.5 rounded bg-[#FBEAE0] text-[#7A2F05] font-semibold">
                                  YOU
                                </span>
                              )}
                            </p>
          ...
          >
            <div className="flex items-center justify-between px-6 py-4 border-b border-[#ECE0D6]">
              <h3 className="text-base font-semibold text-[#1A1A1A] flex items-center gap-2">
                <KeyRound className="w-4 h-4 text-[#C2540C]" />
                Change Role
              </h3>
              <button
                type="button"
                ...
                (data?.logs || []).map((log, idx) => (
                  ...
                  >
                    <td className="px-5 py-3">
                      <span className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-mono font-semibold bg-[#FBEAE0] text-[#7A2F05] border border-[#de7a3d]/50">
                        {log.action}
                      </span>
                    </td>
                    <td className="px-5 py-3">
      ...
      {data && data.pages > 1 && (
        <div className="flex items-center justify-end gap-2">
          <button
            ...
            className="p-2 rounded-lg border border-[#ECE0D6] text-[#6B6B6B] hover:bg-[#FBEAE0] disabled:opacity-40 transition-colors"
          >
    ...
    <div className="admin-panel-page p-6 md:p-8 space-y-6 max-w-[1600px] mx-auto">
      ...
      >
          ...
          <div className="flex items-center gap-2 mb-1.5">
            <span
              className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-semibold border ${
                isSuper
                  ? 'bg-amber-500/10 text-amber-600 border-amber-500/25'
                  : 'bg-[#FBEAE0] text-[#7A2F05] border-[#de7a3d]'
              }`}
            >
        ...
        <button
          ...
          title="Refresh"
        >
          <RefreshCw className={`w-4 h-4 ${isRefetching ? 'animate-spin text-[#C2540C]' : ''}`} />
        </button>
      </motion.div>
      ...
      {tab === 'overview' &&
        (isLoading || !overview ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {[...Array(8)].map((_, i) => (
              <div key={i} className="h-40 rounded-2xl bg-[#FBEAE0] animate-pulse" />
            ))}
          </div>
        ) : (
          <OverviewTab
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between flex-wrap gap-3">
            ...
            <a
              ...
              className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-semibold bg-[#C2540C] hover:bg-[#D06B28] text-white transition-colors"
            >
                    ...
                    permMatrix
                      ...
                      .map((row, idx) => (
                        ...
                        >
                          ...
                          <td className="px-5 py-3">
                            <span className="text-xs font-mono font-semibold text-[#7A2F05] bg-[#FBEAE0] border border-[#de7a3d]/50 px-2 py-0.5 rounded">
                              {row.module}
                            </span>
                          </td>
                          <td className="px-5 py-3">
