  ...
  return (
    <div className="space-y-5">
      ...
      <button
        ...
        className="w-full flex items-center justify-center gap-3 px-4 py-2.5 rounded-lg bg-slate-900 hover:bg-slate-850 border border-slate-800 hover:border-slate-700 text-slate-200 text-sm font-medium transition-all shadow-sm cursor-pointer"
      >
          ...
          {formData.password && (
            <div className="space-y-1.5 pt-1">
              ...
              <div className="grid grid-cols-4 gap-1 h-1.5 w-full bg-slate-900 rounded-full overflow-hidden">
                <div className={`h-full rounded-full transition-all ${passwordScore >= 1 ? strength.color : 'bg-slate-800'}`} />
                <div className={`h-full rounded-full transition-all ${passwordScore >= 2 ? strength.color : 'bg-slate-800'}`} />
                <div className={`h-full rounded-full transition-all ${passwordScore >= 3 ? strength.color : 'bg-slate-800'}`} />
                <div className={`h-full rounded-full transition-all ${passwordScore >= 4 ? strength.color : 'bg-slate-800'}`} />
              </div>
              ...
              <div className="grid grid-cols-2 gap-1 text-[10px] text-slate-400 pt-1">
                <span className={`flex items-center gap-1 ${formData.password.length >= 8 ? 'text-emerald-400' : 'text-slate-500'}`}>
                  {formData.password.length >= 8 ? <Check className="w-3 h-3" /> : <span className="w-3 h-3 text-center">•</span>}
                  8+ characters
                </span>
                <span className={`flex items-center gap-1 ${/[0-9]/.test(formData.password) ? 'text-emerald-400' : 'text-slate-500'}`}>
                  {/[0-9]/.test(formData.password) ? <Check className="w-3 h-3" /> : <span className="w-3 h-3 text-center">•</span>}
                  Numbers included
                </span>
                <span className={`flex items-center gap-1 ${/[A-Z]/.test(formData.password) ? 'text-emerald-400' : 'text-slate-500'}`}>
                  {/[A-Z]/.test(formData.password) ? <Check className="w-3 h-3" /> : <span className="w-3 h-3 text-center">•</span>}
                  Uppercase letters
                </span>
                <span className={`flex items-center gap-1 ${/[^A-Za-z0-9]/.test(formData.password) ? 'text-emerald-400' : 'text-slate-500'}`}>
                  {/[^A-Za-z0-9]/.test(formData.password) ? <Check className="w-3 h-3" /> : <span className="w-3 h-3 text-center">•</span>}
                  Symbols (@$!%*#)
                </span>
              </div>
            </div>
